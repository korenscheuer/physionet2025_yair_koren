...
def run(args):
    ...
    exam_id_to_age = dict()
    exam_id_to_sex = dict()
    exam_id_to_patient = dict()
    exam_id_to_heart_data = dict()
    ...
    for idx, row in df.iterrows():
        ...
        exam_id_to_patient[exam_id] = patient_id

        avb = row['1dAVb']
        assert(is_boolean(avb))
        ...
        exam_id_to_heart_data[exam_id] = {'1dAVb' : avb, 'RBBB' : rbbb, 'LBBB' : lbbb, 'SB' : sb, 'ST' : st, 'AF' : af}
    ...
    for k in range(num_groups):
        ...
        with h5py.File(signal_file, 'r') as f:
            ...
            for i in range(num_exam_ids):
                ...
                patient_id = exam_id_to_patient[exam_id]
                normal_ecg = exam_id_to_normal_ecg[exam_id]
                heart_data = exam_id_to_heart_data[exam_id]
                source = 'CODE-15%'
                comments = [f'Age: {age}', f'Sex: {sex}', f'Chagas label: {chagas}', f'Source: {source}', f'PatientID: {patient_id}', f'NormalECG: {normal_ecg}']

                for heart_key, heart_value in heart_data.items():
                    comments.append('{0}: {1}'.format(heart_key, heart_value))

                # Save the signal.
                record = str(patient_id) + '_' + str(exam_id)
                patient_ids_lst.append(str(patient_id))
                exams_sig_len[record] = digital_signals.shape[0]
    ...
    import pickle
    with open('patient_ids.pkl', 'wb') as f:
        pickle.dump(patient_ids_lst, f)
    with open('sig_len.pkl', 'wb') as f:
        pickle.dump(exams_sig_len, f)
